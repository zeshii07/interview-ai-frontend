...
import { View, Text, StyleSheet, ScrollView, TextInput, TouchableOpacity, KeyboardAvoidingView, Platform, Share, Alert, ActivityIndicator } from 'react-native';
...
const InterviewSession = () => {
  const { 
    currentRole, currentDifficulty, currentQuestion, currentAnswer, currentFeedback, 
    isGeneratingQuestion, isEvaluating, error, setAnswer, generateQuestion, 
    submitAnswer, resetSession, clearError 
  } = useInterviewStore();

  const [questionType, setQuestionType] = useState('mixed');
  
  // Premium Cloud Voice & Share States
  ...
  const handleShareCard = async () => {
    ...
    // EXPO GO WORKAROUND: Bypass screenshot capture which hangs Expo Go
    if (__DEV__) {
      Alert.alert(
        "🎯 Share Feature Ready!", 
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {!currentFeedback && (
          <View style={styles.typeRow}>
            {['mixed', 'behavioral', 'technical', 'situational'].map((type) => (
              ...
              >
                <Text style={[styles.typeBtnText, questionType === type && styles.typeBtnTextActive]}>
                  {type === 'mixed' ? '🎲 Mixed' : type.charAt(0).toUpperCase() + type.slice(1)}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
            ...
            <View style={styles.inputWrapper}>
              ...
              >
                {isTranscribing ? (
                  ...
                  <Ionicons name={isListening ? "mic" : "mic-outline"} size={24} color={isListening ? Colors.error : Colors.textMuted} />
                )}
              </TouchableOpacity>
            </View>
            
            {/* Status Indicators */}
            {isListening && <Text style={styles.listeningText}>🔴 Recording... (Tap mic to stop)</Text>}
            {isTranscribing && <Text style={[styles.listeningText, { color: Colors.primary }]}>⚡ AI is processing your voice...</Text>}
        ...
        {!currentFeedback ? (
          <View style={styles.actions}>
            ...
            <Button title="Submit Answer" onPress={handleSubmit} disabled={currentAnswer.length < 20 || isLoading || isTranscribing} loading={isEvaluating} fullWidth size="large" />
          </View>
        ) : (
        ...
        {currentFeedback && (
          <View style={styles.feedbackContainer}>
            ...
            <View style={{ position: 'absolute', top: -9999, left: -9999 }}>
              <ShareCard ref={shareCardRef} feedback={currentFeedback} role={currentRole} />
            </View>
            ...
            <Button 
              ...
              size="large"
              loading={isSharing}
              icon={<Ionicons name="share-social-outline" size={20} color={Colors.textPrimary} style={{marginRight: 4}} />}
...
const styles = StyleSheet.create({
  ...
  typeBtnText: { color: Colors.textMuted, fontSize: FontSizes.xs, fontWeight: '600' },
  ...
  inputWrapper: { flexDirection: 'row', alignItems: 'flex-end', backgroundColor: Colors.bgCard, borderWidth: 1, borderColor: Colors.border, borderRadius: Radius.lg, overflow: 'hidden' },
  input: { flex: 1, color: Colors.textPrimary, fontSize: FontSizes.md, lineHeight: 24, textAlignVertical: 'top', minHeight: 150, padding: Spacing.md },
  
  micBtn: { padding: Spacing.md, backgroundColor: Colors.bgElevated, borderLeftWidth: 1, borderLeftColor: Colors.border },
  micBtnActive: { backgroundColor: Colors.error + '20' },
  listeningText: { color: Colors.error, fontSize: FontSizes.sm, fontWeight: '600', marginTop: Spacing.xs, marginBottom: Spacing.xs },
